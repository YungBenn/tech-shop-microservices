...
import (
	...
	msg "github.com/YungBenn/tech-shop-microservices/internal/kafka"
...
func buildServer(log *logrus.Logger, db *mongo.Database, address string, Producer *kafka.Producer, KafkaTopic string)  {
	...
	srv := di.InitProductService(db, log, Producer, KafkaTopic)
...
func main() {
	...
	producer, err := msg.NewKafkaProducer(conf)
	if err != nil {
		log.Error("Error connecting to Kafka: ", err)
	}

	productServerUrl := fmt.Sprintf("%s:%s", conf.ProductServiceHost, conf.ProductServicePort)
	buildServer(log, db, productServerUrl, producer.Producer, conf.KafkaTopic)
}